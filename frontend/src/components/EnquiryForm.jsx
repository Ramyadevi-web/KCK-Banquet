...
export const EnquiryForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.eventTypes.length === 0) {
      setMessage({ type: 'error', text: 'Please select atleast one event type' })
      return
    }
    ...
    try {
      ...
      setFormData({ name: '', phone: '', eventTypes: [] })
    } catch (error) {
      setMessage({
        type: 'error',
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label htmlFor="phone" className="block text-brown-50 mb-2 text-sm">
                Your Phone <span className="text-gold-500">*</span>
              </label>
              <input
                id="phone"
                type="tel"
